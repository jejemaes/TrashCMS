...
namespace system\ir;
use system\ir\Registry as Registry;
...
class ModuleLoader{
	...
	public static function loadModules($modules){
		...
		foreach ($loading_order as $module_name){
			...
			// import controller
			$controller_init = _DIR_MODULE . $module->location . 'controller/init.php';
			if(file_exists($controller_init)){
				require_once $controller_init;
			}
			// import models
			$class_before = get_declared_classes();
			$model_init = _DIR_MODULE . $module->location . 'model/init.php';
