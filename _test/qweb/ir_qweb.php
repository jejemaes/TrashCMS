...
/**
 * Maes Jerome
 * ir_qweb.php, created at Dec 23, 2014
 *
 */

/*
 * TODO
 *  - move loader from qwebcontext to engine
 *  - create inteface loader with load($tname)
 *  - replace qwebcontext['key'] by qwebcontext.get('key'), and add default if needed
 */
...
class QWebEngine{
	...
	function get_template($name, $qwebcontext){
		...
		if($this->_loader && !$qwebcontext['template'][$name]){
			$xml_doc = $this->_loader->load($name);
	...
	function eval_format($expr, $qwebcontext){
		...
		*/
		...
		$expr_replaced = preg_replace_callback(
	        ...
	        $expr,
            -1,
            $replacements, $qwebcontext);

		echo 'ee dd : ' . $expr_replaced . '<br>';
		echo "====> ". $replacements;
		echo '<br>';
	...
	function render($id_or_xml_id, $qwebcontext=array(), $loader=FALSE){
		if($loader){
			$this->_loader = $loader;
		}
